...
# 2. 데이터 로드 (파일명 고정 및 전처리)
FILE_NAME = "주요국 통화의 대원화 환율.xlsx - Sheet1.csv"

@st.cache_data
def load_data():
    # 앞선 주석 및 메타데이터 행(상위 8개 행)을 제외하고 데이터프레임 로드
    df = pd.read_csv(FILE_NAME, skiprows=8, names=['날짜', '원/달러', '원/100엔', '원/위안'])
...
try:
    ...
    * **상관계수 해석 기준:**
    ...
    st.markdown("환율의 절대적인 수치 외에, **전일 대비 일일 변동률(Log Return)**을 통해 통계적 왜도와 리스크를 측정합니다.")
