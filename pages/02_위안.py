...
st.set_page_config(page_title="중국 (원/위안) 분석", layout="wide")
...
def load_data():
    ...
    for enc in ['utf-8', 'cp949', 'euc-kr', 'utf-8-sig']:
        try:
            ...
            df['원/위안'] = pd.to_numeric(df['원/위안'], errors='coerce')
            return df[['날짜', '원/위안']].dropna()
        except: continue
    return None
...
if df_raw is not None:
    ...
    if days_to_predict > 0:
        ...
        plot_df['과거 실제 환율 변화'] = pd.Series(df['원/위안'].values, index=df.index)
    ...
    counts, bin_edges = np.histogram(df['원/위안'].pct_change().dropna() * 100, bins=30)
    bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
    hist_df = pd.DataFrame({'원/위안 변화 빈도': counts}, index=np.round(bin_centers, 2))
